<?php

namespace App\Http\Controllers\AM;

use App\Http\Controllers\Controller;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\DB;
use Illuminate\Support\Facades\Auth;
use Illuminate\Support\Facades\Session;

class AmController extends Controller
{
    public function Dashboard() {
        return view("am.dashboard.am_dashboard");
    }


    public function HalamanApproval() {
        $dataApproval = DB::table('table_iwr')
        ->join('master_resto', 'table_iwr.nama_resto', '=', 'master_resto.id')
        ->select('table_iwr.*', 'master_resto.name_store_street as nama_resto')
        ->get();

        return view('am.approval.approval_am', compact('dataApproval'));
    }

    public function UpdateApproval1(Request $request) {
        $request->validate([
            'id' => 'required',
            'approval_1' => 'required'
        ]);

        DB::table('table_iwr')
        ->where('id', $request->id)
        ->update([
            'approval_1' => $request->approval_1,
            'approval_1_username' => Auth::user()->name,
            'approval_1_date' => now()
        ]);
        return response()->json(['success' => true, 'message' => "approval update successfully"]);
    }

    public function logout(Request $request) {
        Auth::logout(); 
        Session::flush(); 
    
        return redirect('/')->with('success', 'You have been logged out.');
    }
}
